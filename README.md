...
Travel Planner - це статична offline-first PWA для планування кількох незалежних подорожей.

Поточна продуктова модель:

- застосунок більше не є тільки Camino-планером;
- Camino del Norte 2026 залишається першою вбудованою подорожжю;
- кожна подорож має власний маршрут, документи, квитки, бюджет, нотатки, щоденник і чек-листи;
- GitHub є джерелом правди для коду і документації;
- Supabase є необов'язковим cloud-шаром для синхронізації між пристроями;
- PWA повинна працювати офлайн після першого завантаження.

Ключовий принцип: усі дані мають читатися і записуватися тільки в контексті active trip. Дані `camino-2026` не повинні потрапляти в `greece-2026` або будь-яку майбутню подорож.

## GitHub Pages

Репозиторій і GitHub Pages:
...
Рекомендована стабільна локальна копія на кожному комп'ютері:

```text
C:\Users\<User>\Documents\Projects\Camino-del-Norte-2026-Planner
```

Поточні старі Codex-копії можна використовувати для продовження цієї сесії, але не варто вважати їх довгостроковим джерелом правди. Не тримаємо важливі зміни тільки локально: commit + push, і тільки тоді інший комп'ютер або телефонна Codex-сесія бачать актуальний стан.
...
- Список подорожей зберігається в `travelplanner_trips`.
...
- Якщо Sync запускається поза вкладкою `Синхр.`, Supabase URL/key беруться зі збереженого config і не перезаписуються порожніми значеннями.
- Якщо Supabase вже підключений, створена Trip одразу зберігається у `trips`; екран `Мої подорожі` може автоматично або вручну підтягнути Trip-метадані з Supabase.
